#!/bin/sh
# Regenerate the terminal logo grid in crates/fluxguard/src/theme.rs from
...
# building FluxGuard never needs either.
set -eu
...
# The shield and flux line only, over a chroma key that marks everything outside
...
# rows, the smallest size at which the flux line inside the shield stays legible.
...
source_file="${repository_root}/crates/fluxguard/src/theme.rs"
