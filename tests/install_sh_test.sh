...
T_ARM64=0
T_OS=Linux T_ARCH=x86_64 && check "linux/x86_64" "$(detect_asset)" "fluxguard-linux-x86_64"
T_OS=Linux T_ARCH=amd64 && check "linux/amd64" "$(detect_asset)" "fluxguard-linux-x86_64"
T_OS=Linux T_ARCH=aarch64 && check "linux/aarch64" "$(detect_asset)" "fluxguard-linux-aarch64"
T_OS=Darwin T_ARCH=arm64 && check "darwin/arm64" "$(detect_asset)" "fluxguard-macos-aarch64"
T_OS=Darwin T_ARCH=x86_64 && check "darwin/x86_64" "$(detect_asset)" "fluxguard-macos-x86_64"
T_OS=Darwin T_ARCH=x86_64 T_ARM64=1 &&
	check "darwin/rosetta" "$(detect_asset)" "fluxguard-macos-aarch64"
T_ARM64=0
T_OS=Linux T_ARCH=riscv64 &&
	check "rejects unknown arch" "$(detect_asset 2>&1 || true)" "fluxguard: unsupported architecture: riscv64"
T_OS=Plan9 T_ARCH=x86_64 &&
	check "rejects unknown os" "$(detect_asset 2>&1 || true)" "fluxguard: unsupported operating system: Plan9"
T_OS=MINGW64_NT-10.0 T_ARCH=x86_64 &&
	check "points Windows at the ps1" "$(detect_asset 2>&1 | cut -d';' -f1 || true)" \
		"fluxguard: Windows is not supported by this script"
...
aaa11  fluxguard-linux-x86_64
bbb22  fluxguard-linux-aarch64
ccc33 *fluxguard-macos-aarch64
SUMS
look() { grep " \{1,2\}\*\{0,1\}$1\$" "$work/SHA256SUMS" | cut -d' ' -f1; }
check "sums exact match" "$(look fluxguard-linux-x86_64)" "aaa11"
check "sums aarch64 entry" "$(look fluxguard-linux-aarch64)" "bbb22"
check "sums binary-mode entry" "$(look fluxguard-macos-aarch64)" "ccc33"
check "sums missing asset" "$(look fluxguard-macos-x86_64)" ""
check "sums no prefix bleed" "$(look fluxguard-linux)" ""
...
mkdir -p "$rel"
printf '#!/bin/sh\necho installed-ok\n' >"$rel/$asset"
(cd "$rel" && (sha256sum fluxguard-* 2>/dev/null || shasum -a 256 fluxguard-*) >SHA256SUMS)
...
run() { FLUXGUARD_VERSION=v9.9.9 FLUXGUARD_INSTALL_DIR="$bin" sh "$work/local.sh" 2>&1 | grep -v '^curl:' || true; }

out=$(run)
check "installs the binary" "$(sh "$bin/fluxguard")" "installed-ok"
...
check "rejects checksum mismatch" "$(run | grep -c '^fluxguard: checksum mismatch')" "1"
